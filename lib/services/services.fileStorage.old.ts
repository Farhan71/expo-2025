...
// Read services from file (private helper)
function _readServicesFromFile(): Service[] {
  ensureDataDir();
...
// Helper functions for synchronous access (for SSR pages)
export function readServicesFromFile(): Service[] {
  return _readServicesFromFile();
}

export function getActiveServicesFromFile(): Service[] {
  return _readServicesFromFile()
    .filter((service) => service.active)
    .sort((a, b) => a.order - b.order);
}

export function getServiceBySlugFromFile(slug: string): Service | undefined {
  return _readServicesFromFile().find((service) => service.slug === slug);
}

// Async CRUD operations for API routes
...
export async function getServiceBySlug(slug: string): Promise<Service | null> {
  try {
    const service = getServiceBySlugFromFile(slug);
    return service || null;
  } catch (error) {
...
export async function createService(
  ...
  try {
    const services = _readServicesFromFile();
