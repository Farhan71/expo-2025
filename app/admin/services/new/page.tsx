...
export default function AdminNewServicePage() {
  ...
  const handleInputChange = (field: keyof ServiceCreate, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate form data
      const validatedData = ServiceCreateSchema.parse(formData);

      // Send to API endpoint to create service
      ...
      if (response.ok) {
        ...
        alert(
          'Service created successfully! (Demo mode - not actually saved to data file)'
        );
        ...
        if (errorData.details) {
          ...
          alert(`Failed to create service: ${errorData.error}`);
        }
      }
    } catch (error: any) {
      if (error.errors) {
